import pickle

import pytest
import numpy as np

from chebpy import chebfun, pwc
from chebpy.core.settings import DefaultPreferences


def test_chebfun_null_args():
    assert chebfun().isempty


def test_chebfun_callable():
    n = 100
    d = np.array([-2, 0, 1])
    f1 = chebfun(np.sin)
    f2 = chebfun(np.sin, d)
    f3 = chebfun(np.sin, n=n)
    f4 = chebfun(np.sin, d, n)

    # check domains
    assert f1.domain == DefaultPreferences.domain
    assert f2.domain == d
    assert f3.domain == DefaultPreferences.domain
    assert f4.domain == d

    # check lengths of f3 and f4
    assert f3.funs[0].size == n
    assert np.all([fun.size == n for fun in f4])


def test_chebfun_alphanum_char():
    n = 100
    d = np.array([-2, 0, 1])
    f1 = chebfun("x")
    f2 = chebfun("y", d)
    f3 = chebfun("z", n=n)
    f4 = chebfun("a", d, n)

    # check domains
    assert f1.domain == DefaultPreferences.domain
    assert f2.domain == d
    assert f3.domain == DefaultPreferences.domain
    assert f4.domain == d

    # check lengths of f3 and f4
    assert np.sum([fun.size for fun in f3]) == n
    assert np.all([fun.size == n for fun in f4])


def test_chebfun_float_arg():
    d = np.array([-2, 0, 1])
    f1 = chebfun(3.14)
    f2 = chebfun("3.14")
    f3 = chebfun(2.72, d)
    f4 = chebfun("2.72", d)

    # check domains
    assert f1.domain == DefaultPreferences.domain
    assert f2.domain == DefaultPreferences.domain
    assert f3.domain == d
    assert f4.domain == d

    # check all are constant
    assert f1.isconst
    assert f2.isconst
    assert f3.isconst
    assert f4.isconst


def test_chebfun_raises():
    with pytest.raises(ValueError):
        chebfun("asdfasdf")


def test_pwc():
    dom = [-1, 0, 1]
    vals = [0, 1]
    f = pwc(dom, vals)
    assert f.funs.size == 2
    for fun, val in zip(f, vals):
        assert fun.isconst
        assert fun.coeffs[0] == val


@pytest.fixture
def pickled_chebfun():
    f0 = chebfun(np.sin, [-2, 0, 1])
    f1 = pickle.loads(pickle.dumps(f0))
    return f0, f1


def test_evaluate(pickled_chebfun):
    f0, f1 = pickled_chebfun
    x = -1
    assert f0(x) == f1(x)

# TODO: implement test for equality once objects can be compared
# def test_equality(pickled_chebfun):
#     f0, f1 = pickled_chebfun
#     assert vars(f0) == vars(f1)
